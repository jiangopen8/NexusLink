...
describe('SettlementModule - local ledger fallback', () => {
  it('sends USDC and updates local balances without chain env', async () => {
    const oldSettlement = process.env.SETTLEMENT_ADDRESS;
    const oldAgent = process.env.AGENT_DID;
    delete process.env.SETTLEMENT_ADDRESS;
    process.env.AGENT_DID = 'did:nexus:sender';
    const dir = mkdtempSync(join(tmpdir(), 'nexuslink-settlement-'));
    const settlement = new SettlementModule(new ConfigStore(join(dir, 'config.yaml')), join(dir, 'ledger.json'));
    const tx = await settlement.send('did:nexus:receiver', '25.5');
    expect(tx.txHash).toMatch(/^0x/);
    expect(await settlement.balance('did:nexus:receiver')).toBe('25.500000 USDC');
    expect(await settlement.balance('did:nexus:sender')).toBe('974.500000 USDC');
    if (oldSettlement) process.env.SETTLEMENT_ADDRESS = oldSettlement;
    if (oldAgent) process.env.AGENT_DID = oldAgent;
  });
});
...
describe('SettlementModule - Multi-currency', () => {
  ...
  it('getBalance CNY returns local gateway balance with CNY', async () => {
