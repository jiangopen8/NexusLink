...
export const DID_SKILLS: Record<string, SkillDescriptor> = {
  ...
  'nexuslink:did:update': {
    skill: 'nexuslink:did:update',
    version: '1.0.0',
    name: 'Update DID Document',
    description: 'Update skills and languages on a DID document',
    intent: { triggerPatterns: ['update DID', 'update agent profile', 'update profile'], category: 'identity' },
    parameters: { type: 'object', properties: { did: { type: 'string' }, skills: { type: 'array', items: { type: 'string' } }, languages: { type: 'array', items: { type: 'string' } } }, required: ['did'] },
    returns: { type: 'object', properties: { id: { type: 'string' } } },
  },
  'nexuslink:did:set-boundary': {
    skill: 'nexuslink:did:set-boundary',
    version: '1.0.0',
    name: 'Set Intent Boundary',
    description: 'Set autonomous action boundaries for an agent DID',
    intent: { triggerPatterns: ['set permissions', 'set intent boundary', 'set autonomy limits'], category: 'identity' },
    parameters: { type: 'object', properties: { did: { type: 'string' }, allowedDomains: { type: 'array', items: { type: 'string' } }, maxValueUsdc: { type: 'number' } }, required: ['did'] },
    returns: { type: 'object', properties: { id: { type: 'string' }, intentBoundary: { type: 'object' } } },
  },
};
