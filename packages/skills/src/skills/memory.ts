...
export const MEMORY_SKILLS: Record<string, SkillDescriptor> = {
  ...
  'nexuslink:memory:sync': {
    skill: 'nexuslink:memory:sync',
    version: '1.0.0',
    name: 'Sync Memory Index',
    description: 'Summarize memory shards for backend migration and synchronization',
    intent: { triggerPatterns: ['sync memory', 'memory migration', 'memory index'], category: 'memory' },
    parameters: { type: 'object', properties: {} },
    returns: { type: 'object', properties: { shards: { type: 'number' }, bytes: { type: 'number' } } },
  },
};
