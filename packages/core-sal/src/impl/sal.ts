...
export class SALModule {
  ...
  constructor(options: SALOptions) {
    if (options.backend === 'local') {
      this.backend = new LocalSAL(options.localPath);
    } else if (options.backend === 'ipfs') {
  ...
  getBackendType(): string {
    if (this.backend instanceof LocalSAL) return 'local';
  ...
  supportsFeature(feature: 'fast-retrieval' | 'large-files' | 'encryption'): boolean {
    ...
    switch (feature) {
      case 'fast-retrieval':
        return type === '0g' || type === 'local';
      case 'large-files':
        return true;
      case 'encryption':
        return type === 'ipfs' || type === 'local';
      default:
        return false;
    }
  }
